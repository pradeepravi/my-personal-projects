...
public class MovieRecommendationsServiceImpl implements RecommendationService {
	...
	public List<Recommendation> getRecommendations(Map<RecommendationParam, List<String>> params) throws NoSuchMethodException {
	...
	private List<Recommendation> getRecommendationsForMoviesAlreadySeen(List<String> movieTitles) {
 		...
 		try {
 			...
 			fetchedDetails.stream().filter(m -> (m.getMovieDetails() != null && m.getMovieDetails().getGenres() != null))
 				.forEach(n -> {
	...
	@Override
	public List<Recommendation> getRecommendations(Map<RecommendationParam, List<String>> parameters, List<String> exclusions) throws NoSuchMethodException {
